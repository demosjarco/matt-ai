...
export default class extends WorkerEntrypoint<EnvVars> {
	...
	async messageAction(message: string, longer: boolean) {
		...
		const response = await translator.translate(message, [
			...
			{ role: 'system', content: 'If the user asks to search online (ignore any and all provided direct links). If you do not directly know the answer or the topic is outside of your knowledge, provide appropriate search terms' },
