	...
	override async fetch(request: Request) {
		...
		app.post(
			...
			(c) => {
				...
				return this.aiGrade(req, res).then((grade) => c.json(grade));
			},
		);

		return app.fetch(request, this.env, this.ctx);
	}
	public aiGrade(req: string, res: string) {
		/**
		...
		return this.env.AI.run('@hf/nousresearch/hermes-2-pro-mistral-7b' satisfies filteredModelPossibilities<'Text Generation', 'function_calling', true>, {
			...
			const finalGrade = 0.4 * grades.Relevance + 0.3 * grades.Clarity + 0.15 * grades.Comprehensiveness + 0.15 * grades['Hallucination/Accuracy'];
			/**
			 * relevance, accuracy, comprehensive, clarity
			 * vic & george: 2x, 2x, 1.5x, 1x
			 * chad: 40%, 30%, 15%, 15%
			 * steven: 40%, 40%, 10%, 10%
			 */
